...
```
Steps to Reproduce:
1. Register a MailboxRouteMongo/MailboxRouteSQL with the server
2. POST a new Mailbox with primarySmtpAddress `user@example.com`
3. Send an inbound message to `user@example.com` via the configured MTA
4. Run the rapidrest server (`yarn start`)
5. GET `/mailboxes/:id/folders/inbox/messages`
6. Observe error

Repro Rate: 3/3

Expected:
The newly delivered message to appear in the response with the following:
// Insert mockup, description details

Actual:
The response returned wtih the following:
// Insert screenshot, log output

Severity: BLOCKER

Project Info:
@rapidrest/mail: v1.0.0
...
```
As a developer I would like the ability to configure a per-mailbox retention policy for quarantined messages.
Administrators would set a retention window per organization instead of relying on the global default.
```
